...
**Resolution:** `home/default.nix` still imports `vicinae.homeManagerModules.default` so the option remains available if needed later, but `home/packages.nix` installs `pkgs.vicinae` through `vicinaePkg` and does not enable `services.vicinae`. `config/hypr/autostart.lua` starts `vicinae server` directly, while `SUPER+R` / `vicinae open` only opens the already-running launcher. The `nrs` function in `home/shell.nix` asks Hyprland to run `vicinae server --replace` after a successful activation, refreshing the launcher without introducing a second systemd-owned service. This restart matters after desktop-entry ID changes in particular, which the long-lived Vicinae server only sees after a replace.

## Home Manager symlink swaps trip Hyprland's config watcher
**Symptom:** Without a guard, an activating rebuild reloads the Hyprland config repeatedly mid-activation, and some of those reloads see a config whose `require`d files are momentarily absent.
**Cause:** Home Manager replaces the entries under `~/.config/hypr/` one at a time, and Hyprland's watcher treats a replaced symlink as a config change just like a direct write. Measured on Hyprland 0.56.1 in a nested instance: swapping a symlink whose new target changed `general:border_size` from 7 to 13 applied within seconds, the same as an in-place write to a `require`d file or to the top-level config.
**Status:** Workaround in place
**Resolution:** The `nrs` function in `home/shell.nix` sets `misc:disable_autoreload` for the duration of an activating rebuild and restores the value it found. The restore is registered as `trap ... EXIT INT TERM`, not `trap ... EXIT`: a zsh EXIT trap alone does not run when SIGINT kills the shell, so an EXIT-only guard would leave autoreload disabled for the rest of the session after a Ctrl-C.
...
**Status:** Workaround in place
**Resolution:** `home/shell.nix` makes the `nrs` wrapper pass the target `system-features` list directly to `sudo nixos-rebuild` whenever native optimizations are enabled. That bootstraps the native-tagged derivations through the current daemon. Plain non-root `nix build --option system-features ...` is not sufficient here because `system-features` is a restricted setting for untrusted users.
...
**Status:** Workaround in place
**Resolution:** The `neuwaita` derivation in `home/gtk.nix` installs upstream-shaped `Neuwaita` for GTK, whose vendored `index.theme` already carries comma-only separators in its inherited-theme list, and a derived `Neuwaita-KDE` wrapper with `FollowsColorScheme=true` plus `Inherits=Neuwaita,breeze,Adwaita,hicolor`. Both installed themes get aliases for common folder names that upstream Neuwaita lacks, including `folder-blue`. The Qt target maps the shared `Neuwaita` state value to that wrapper for KDE only.

## Neuwaita's upstream repository no longer exists
**Symptom:** A rebuild that re-evaluates the `neuwaita` derivation fails in `fetchFromGitHub` with repeated `curl: (22) The requested URL returned error: 404` on `RusticBard/Neuwaita/archive/112525b2….tar.gz`, followed by `error: cannot download source from any mirror`. Nothing in the config changed; bumping `flake.lock` is enough to trigger it, because a new `stdenv` changes the derivation hash and forces the source to be fetched again instead of reused from the store.
**Cause:** The upstream project was deleted outright — the `RusticBard` GitHub account itself 404s, so the repo, its commits, and its fork network are all gone. Pinned rev `112525b2a97226bb3e8ef4433c039bc514bc2973` is unreachable from every remote: no surviving fork carries it, Software Heritage never archived the origin, the theme is not in nixpkgs, and the fixed-output source path is not on `cache.nixos.org`. No re-pin or hash update can recover it.
**Status:** Workaround in place
**Resolution:** The theme was recovered from the last surviving local build output of that rev and vendored into the repo as `home/neuwaita/neuwaita-112525b2.tar.xz` (~815K, built reproducibly with sorted names and a fixed mtime). `home/gtk.nix` now uses that tarball as `src` instead of `fetchFromGitHub`. The five folder-name aliases the `installPhase` adds were stripped back out of the vendored tree, so it keeps the upstream shape the derivation expects and the install logic is unchanged. Treat this tarball as the only extant copy of the theme — there is nowhere left to re-fetch it from.
...
**Symptom:** Every `nixos-rebuild switch` ended with `warning: the following units failed: desktopctl-sddm-theme-sync.path, desktopctl-sddm-theme-sync.service` and a non-zero exit, even though the journal shows the sync itself succeeding (`Finished Sync desktopctl wallpaper into SDDM background`) immediately before `Start request repeated too quickly`.
**Cause:** `systemd.paths.desktopctl-sddm-theme-sync` triggers on `PathExists`, which is level-triggered, not edge-triggered: the path unit re-arms as soon as the triggered service finishes and fires again while the file is still present. The sync copied `/tmp/desktopctl-where-is-my-sddm-theme/background` into place but never removed it, so the condition stayed true forever and the service span until systemd's start limiter tripped and failed both units. The service exiting `0/SUCCESS` is what makes this confusing — nothing about the sync is broken, only the retrigger.
**Status:** Fixed
**Resolution:** The `ExecStart` script in `system/services.nix` now consumes the staging file via `trap 'rm -f ...' EXIT`, the standard systemd spool-directory pattern. The trap fires on failure too, deliberately: a file that cannot be installed would otherwise loop forever, and `desktopctl` re-stages on every theme apply, so a dropped sync self-corrects on the next one. Any future `PathExists` unit in this repo needs the same discipline — either consume the file or use `PathChanged` alone.
