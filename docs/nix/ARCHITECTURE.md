...
| --- | --- | --- |
| `system/configuration.nix` | Shared system root module | Nix settings including the `cache.nixos.org` and Vicinae substituters, the shared unfree allowlist and narrow insecure-package exceptions used by both NixOS and embedded Home Manager, the local package overlay plus the `claudeCodeOverlay` received from `flake.nix`, Hyprland packaging, the repo-wide fontconfig baseline, shared non-Qt session env, base system packages, locale/docs, and imports of the concern-specific shared system modules |
