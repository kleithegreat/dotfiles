...
| --- | --- |
| Outputs | The `outputs` attrset in `flake.nix` exports `nixosConfigurations.laptop`, `nixosConfigurations.desktop`, plus `overlays.default`, `devShells.x86_64-linux.default`, `packages.x86_64-linux.desktopctl`, `packages.x86_64-linux.helium`, and `packages.x86_64-linux.snappy-switcher` |
...
| SDDM background bridge | `system/services.nix` seeds `/var/lib/desktopctl/where-is-my-sddm-theme/background` from `wallpapers/lmao.png`, points the `where_is_my_sddm_theme` package override at that persistent file with blur enabled, and uses `desktopctl-sddm-theme-sync.path` / `.service` to move the user-staged `/tmp/desktopctl-where-is-my-sddm-theme/background` file into the root-owned SDDM-readable location. The sync deletes the staging file once it has read it: the path unit triggers on `PathExists`, which re-fires while the file remains, so the handoff has to be consumed rather than copied (see `QUIRKS.md`). A `systemd.tmpfiles.rules` entry pre-creates that staging directory at boot as `0700 kevin kevin` (via the `sddmThemeStagingDir` binding), so no other local user can pre-create it or symlink-swap the staged file the root-run sync copies. |
