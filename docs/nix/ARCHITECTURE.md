...
| `pkgs/openchamber-backend-mux/default.nix` | Mixed backend mux package | Wraps `pkgs/openchamber-backend-mux/index.mjs`, a lightweight HTTP multiplexer that starts both OpenCode and the Claude bridge, merges provider/model/session listings, routes session-specific calls by session ID, and lets one OpenChamber runtime host chats pinned to different backends |
