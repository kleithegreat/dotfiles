...
| --- | --- |
| Outputs | The `outputs` attrset in `flake.nix` exports `nixosConfigurations.laptop`, `nixosConfigurations.desktop`, plus `overlays.default`, `packages.x86_64-linux.desktopctl`, `packages.x86_64-linux.helium`, `packages.x86_64-linux.openchamber`, `packages.x86_64-linux.openchamber-claude-bridge`, and `packages.x86_64-linux.openchamber-backend-mux` |
...
| `system/services.nix` | Shared service module | PKI trust, firewall defaults, XDG portals, SDDM plus the custom `where_is_my_sddm_theme` package install and the root-owned `/tmp`-to-`/var/lib/desktopctl/where-is-my-sddm-theme/background` sync bridge that keeps the greeter background aligned with the staged `desktopctl` wallpaper, root-only weekly `fstrim`, PipeWire/WirePlumber, disabled-by-default Bluetooth/Blueman, printing/Avahi, and Samba declarations, Docker/libvirt, GeoClue, Tailscale, Mullvad, gnome-keyring, GVFS, dconf, Partition Manager, and the system-level Bitwarden install required for polkit wiring |
...
| `home/packages.nix` | Shared package module | The shared `home.packages` selection for CLI tools, desktop apps, editors including Zed, and media tooling, including `brightnessctl` plus `ddcutil` for the unified brightness path, plus Vicinae as an installed launcher without the Home Manager service enabled; the KDE app set explicitly includes both `kdePackages.plasma-systemmonitor` and `kdePackages.ksystemstats` so the monitor's session D-Bus backend is present even outside a full Plasma session |
...
| `home/gtk.nix` | GTK submodule | GTK packages, the repo-packaged Neuwaita icon theme patched with KDE `FollowsColorScheme=true` plus Breeze-before-Adwaita fallback ordering, and small dconf defaults |
...
| `pkgs/openchamber-backend-mux/default.nix` | Mixed backend mux package | Wraps `pkgs/openchamber-backend-mux/index.mjs`, a lightweight HTTP multiplexer that starts both OpenCode and the Claude bridge, normalizes mixed provider catalogs (including OpenCode's plain-string provider IDs), merges provider/model/session listings plus the global `/experimental/session` pages the sidebar consumes, forwards selected `model` / `variant` / `agent` fields when creating new sessions, routes session-specific calls by session ID, and lets one OpenChamber runtime host chats pinned to different backends |
| `overlays/local-packages.nix` | Local package overlay | Exposes the repo's `desktopctl`, `helium`, `snappy-switcher`, `openchamber`, `openchamber-cli`, `openchamber-desktop`, `openchamber-claude-bridge`, and `openchamber-backend-mux` derivations and carries the repo-local `sf-pro` font package |

## Overlay Usage

| Surface | Current implementation |
| --- | --- |
| `overlays/local-packages.nix` | Exposes the repo's packaged apps (`desktopctl`, `helium`, `snappy-switcher`, `openchamber`, `openchamber-cli`, `openchamber-desktop`, `openchamber-claude-bridge`, `openchamber-backend-mux`) and carries the repo-local `sf-pro` font derivation. |
...
| Shared native overlay | `overlays/native-optimized.nix` rebuilds selected nixpkgs packages (`desktopctl`, `pipewire`, `quickshell`, `fd`, `ripgrep`, and the repo's TeX Live environment) with host-native flags while deliberately leaving low-level rebuild multipliers such as `zstd` and `lz4` stock. |
...
| Home Manager consumers | `home/default.nix` now takes Snappy Switcher from the repo-local `pkgs.snappy-switcher` package exported through `overlays/local-packages.nix`, which fetches the upstream source snapshot and applies `patches/snappy-switcher/workspace-scope-filter.patch` directly instead of routing through a separate flake input. Vicinae keeps the upstream flake input only for the optional `vicinae.homeManagerModules.default` option surface, while `home/packages.nix` installs the cached `pkgs.vicinae` package directly and leaves `services.vicinae` disabled because Hyprland owns `vicinae server` startup. OpenCode now comes straight from `pkgs.opencode` in the pinned `nixpkgs` set instead of the old upstream-flake source override, and the same module imports `inputs.nixpkgs-stable` for one targeted exception: `harunaPkg = stablePkgs.haruna`, which `home/packages.nix` installs in place of unstable `pkgs.haruna` to stay on a cacheable player build and avoid the current unstable `yt-dlp -> deno -> rusty_v8` rebuild path. `home/packages.nix` then installs the selected host-native user packages explicitly. |
...
| Shared physical-host VM runtime | `system/windows-vm.nix` installs the `windows-vm` launcher, keeps the Windows guest state rooted at `/var/lib/windows-vm/windows11` on both physical hosts, prefers the staged `windows11.iso` path but can fall back to a lone `.iso` in the same directory, attaches `unattend.iso` when present, and exposes the installer through a boot-prioritized SATA CD-ROM device. |
| Desktop-only runtime | `hosts/desktop/system.nix` keeps the NVIDIA policy, forced performance profile, desktop-specific kernel trim, the extra desktop VM writeback/cache-pressure sysctls, and enables the `hosts/desktop/macos-vm.nix` Sequoia VM helpers for the desktop only. |
