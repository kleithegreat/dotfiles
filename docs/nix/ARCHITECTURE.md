...
| Host constructor | `mkHost` in `flake.nix` wraps `nixpkgs.lib.nixosSystem`, passes the shared `host` fact record and the single `claudeCodeOverlay` through NixOS `specialArgs`, and passes only Home Manager-consumed arguments through `home-manager.extraSpecialArgs` |
...
| --- | --- | --- |
| `system/configuration.nix` | Shared system root module | Nix settings including the `cache.nixos.org` and Vicinae substituters, the shared unfree allowlist (which carries the CUDA 13 redistributable, cuDNN, NVSHMEM, `torch`, and `triton` names ComfyUI's PyTorch pulls in alongside the pre-existing CUDA 12 entries) and narrow insecure-package exceptions used by both NixOS and embedded Home Manager, the local package overlay plus the `claudeCodeOverlay` received from `flake.nix`, Hyprland packaging, the repo-wide fontconfig baseline, shared non-Qt session env, base system packages, locale/docs, and imports of the concern-specific shared system modules |
...
| `pkgs/openchamber/cli.nix`, `pkgs/openchamber-desktop/default.nix`, and `pkgs/openchamber/default.nix` | Source-built OpenChamber package set | `pkgs/openchamber/cli.nix` fetches OpenChamber 1.16.3, applies the consolidated `patches/openchamber/backend-integration.patch` plus the desktop popup-performance patch, restores the trimmed repo-pinned root npm manifest/lock, applies upstream's TanStack virtual-core Bun patch explicitly for npm, builds `packages/web`, and wraps the CLI with store paths to the Claude bridge and mixed-backend mux. The backend patch integrates `opencode`, `claude-code`, and `mixed` modes with upstream's current process registry, HMR state, directory routing, session metadata, and model/provider-aware creation. `pkgs/openchamber-desktop/default.nix` builds the local Tauri shell, reuses its remembered port, rejects ports claimed by another runtime, refocuses through the single-instance plugin, opens an undecorated WebKitGTK window, and asynchronously authenticates and posts `/api/system/shutdown` on exit. Its wrapper disables WebKitGTK dmabuf rendering, and its `openchamber-desktop.desktop` ID plus `StartupWMClass=openchamber-desktop` match the Wayland app_id. `pkgs/openchamber/default.nix` publishes the CLI and desktop app together as a `symlinkJoin`. |
...
| `pkgs/openchamber-backend-mux/default.nix` | Mixed backend mux package | Wraps `pkgs/openchamber-backend-mux/index.mjs`, pins both packaged backend binaries, runs the local Node test suite during the derivation check phase, starts OpenCode plus the Claude bridge, and cleans up partial startup. It normalizes and merges provider/model/session catalogs plus global `/experimental/session` pages, forwards session `metadata` together with selected `model` / `variant` / `agent`, and persists each session's backend binding. Merged endpoints and SSE degrade while either backend remains healthy; a child death terminates its sibling so the lifecycle can restart the pair, and shutdown closes held connections before killing both children. |
...
The Claude bridge stores OpenChamber session metadata in the existing persisted
session info and implements the session PATCH metadata update path, so review
markers and links survive bridge restarts. Its Nix derivation runs focused
create, update, and reload-persistence tests during `checkPhase`; the backend
mux check likewise covers backend-specific model translation and model-less
...
| --- | --- |
| `overlays/local-packages.nix` and `flake.nix` `claudeCodeOverlay` | Exposes the repo's packaged apps (`comfyui`, `desktopctl`, `helium`, `snappy-switcher`, `openchamber`, `openchamber-cli`, `openchamber-desktop`, `openchamber-claude-bridge`, `openchamber-backend-mux`), carries the repo-local `sf-pro` font derivation, replaces `claude-code` with the package from the narrow `nixpkgs-claude` input so Claude Code can track model-support releases faster than `nixos-unstable`, replaces `lmstudio` with the upstream `0.4.20-1` x64 AppImage, and keeps Bambu Studio on the stable upstream `02.07.01.62` AppImage rather than the newer public-beta source package in locked nixpkgs. The AppImage desktop files are rewritten to absolute `$out/bin/lm-studio-desktop` and `$out/bin/bambu-studio-desktop` shell-parent launchers, keeping Vicinae's detached launcher from orphaning AppImage/FHS wrapper paths while still avoiding ambient `PATH` lookup for the final binaries; Bambu Studio's desktop metadata is also normalized to `StartupWMClass=BambuStudio`, matching the class Hyprland reports for the real window. |
...
| Shared system consumers | `system/configuration.nix` applies the repo-local package overlay plus the `claudeCodeOverlay` received from `flake.nix` globally, uses a local `optimizedPkgs` set for PipeWire/WirePlumber, reuses `system/native-optimizations.nix` directly for the patched Hyprland-family derivations, keeps `hyprbars` on the rolling `hyprland-plugins` flake package, and builds `hyprexpo` from `pkgs/hyprland-plugins/hyprexpo/default.nix` because upstream removed that package output. |
| Home Manager consumers | `home/default.nix` takes Snappy Switcher from the repo-local `pkgs.snappy-switcher` package exported through `overlays/local-packages.nix`; upstream v4 now supplies workspace filtering directly, and the Hyprland bindings select it with `--workspace --mod alt` instead of a local source patch. Vicinae keeps the upstream flake input only for the optional `vicinae.homeManagerModules.default` option surface, while `home/packages.nix` installs the cached `pkgs.vicinae` package directly and leaves `services.vicinae` disabled because Hyprland owns `vicinae server` startup. OpenCode and Haruna come straight from `pkgs.opencode` and `pkgs.haruna` in the pinned `nixpkgs` set instead of package-specific source overrides; Haruna must stay on the same Qt/KDE package set as the session-wide `hyprqt6engine` platform theme plugin. `home/packages.nix` then installs the selected host-native user packages explicitly. |
...
- `system/services.nix` enables `programs.partition-manager`,
