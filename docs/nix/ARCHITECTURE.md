...
| `home/packages.nix` | Shared package module | The shared `home.packages` selection for CLI tools, desktop apps, editors including Zed, and media tooling, including `brightnessctl` plus `ddcutil` for the unified brightness path, plus the Home Manager-managed Vicinae service package wiring; the KDE app set explicitly includes both `kdePackages.plasma-systemmonitor` and `kdePackages.ksystemstats` so the monitor's session D-Bus backend is present even outside a full Plasma session |
