...
| `home/packages.nix` | Shared package module | The shared `home.packages` selection for CLI tools, desktop apps, editors including Zed, and media tooling, including `brightnessctl` plus `ddcutil` for the unified brightness path, plus Vicinae as an installed launcher without the Home Manager service enabled, and `mission-center` as the GTK system monitor |
