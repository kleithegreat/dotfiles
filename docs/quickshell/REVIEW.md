...
| Medium | `system_font` has no visible effect on shell chrome. | `desktopctl`'s `quickshell` target emits both `family` (from `mono_font`) and `systemFamily` (from `system_font`) in `GeneratedTheme.json`. `Theme.qml` exposes them as `fontFamily` and `systemFamily`. QML binds `Theme.fontFamily` across the shell UI; `Theme.systemFamily` is only referenced in `SettingsFontsPane.qml` and `SettingsPresetEditor.qml`. Changing `system_font` updates `GeneratedTheme.json` but has no visible effect on bar, popups, notifications, or other shell surfaces. |
