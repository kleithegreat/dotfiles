...
| Schema | `desktopctl/src/theme/schema.rs:121-580` defines `ColorScheme`, required `appearance`, centralized per-target app-theme metadata including KTextEditor theme names, `ThemeState`, canonical field ordering, compiled default theme-state values, and the shared mono-font-size helpers. `desktopctl/src/theme/schema.rs:423-466` now derives the default `dark_hint` from the default scheme's declared appearance instead of a detached constant. |
