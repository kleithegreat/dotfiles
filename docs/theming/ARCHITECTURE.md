...
| Schema | `desktopctl/src/theme/schema.rs` defines `ColorScheme`, required `appearance`, the optional `dark_scheme` pairing key, centralized per-target app-theme metadata including KTextEditor theme names, `ThemeState`, canonical field ordering, compiled default theme-state values, the per-target system-font and mono-font offset contract, and the shared font-size / mono-font-size helpers. The default-state logic there still derives `dark_hint` from the default scheme's declared appearance instead of a detached constant. |
| Resolution | `desktopctl/src/theme/resolve.rs` resolves `themes/colors/`, rejects schemes that omit `appearance`, persists theme state in the shared `desktopctl.db` `theme_state` table, backfills missing required `ThemeState` keys from compiled defaults when older SQLite rows or legacy `themes/state.json` inputs are reused, canonicalizes known legacy string aliases such as older mono-font labels before validation/persistence, imports the legacy JSON on first access, and serializes canonical JSON for CLI output. Mutations persist only the changed keys through `save_state_keys()` upserts in one transaction; full-table writes remain only for seed and backfill paths. |
| JSON compatibility | `desktopctl/src/theme/json.rs` preserves Python-compatible object ordering and ASCII escaping for generated JSON files and `--json` CLI output, including `theme status --json`. |
...
| Inspection | `list-schemes`, `list-presets`, `status` | Human-readable text by default, with Quickshell-facing `--json` modes that return deterministic array/object shapes and the canonical theme-state JSON order from SQLite-backed storage. Human and `--json` listings share one filename-sorted ordering (`json_file_stems_by_filename`). |
...
| `command` | `chromium`, `gtk`, `helium`, `wallpaper`, `where_is_my_sddm_theme` |
...
| --- | --- |
| `chromium` | `desktopctl/src/theme/targets/chromium.rs` is command-only and patches each active Chromium profile's `Preferences` file in place by reading `Local State` `profile.last_active_profiles`, falling back to `Default`, recursively preserving unrelated profile prefs while setting the common-script web font families, forcing `browser.theme.color_scheme2` from `dark_hint`, removing custom browser color keys managed by Chrome's color picker, and removing any previously managed page-size prefs so Chromium falls back to its own defaults. |
| `cursor` | `desktopctl/src/theme/targets/cursor.rs` writes cursor index files plus `~/.config/hypr/cursor-theme.lua`, updates dconf, updates Hyprland cursor env, and imports cursor vars into the user environment. |
...
| `helium` | `desktopctl/src/theme/targets/helium.rs` is command-only and reuses the Chromium-family active-profile preference patching for Helium's `~/.config/net.imput.helium/` user data directory, so Helium gets the same web font families and `dark_hint`-driven browser chrome preference as Chromium. |
| `neovim` | `desktopctl/src/theme/targets/neovim.rs` writes `~/.config/nvim/lua/theme-state.json` with only `background`, derived from `ColorScheme.appearance`; `config/nvim/lua/plugins/colors.lua` always loads the installed Gruvbox scheme. |
| `qt` | `desktopctl/src/theme/targets/qt.rs` mirrors the palette into qt5ct, qt6ct, KDE, hyprqt6engine, Kvantum, Kate, and KWrite. The target writes `[Icons] Theme` into `kdeglobals`, maps the shared `Neuwaita` selection to the KDE-only `Neuwaita-KDE` wrapper icon theme from `home/gtk.nix`, drives KTextEditor from `ColorScheme.app_themes.ktexteditor`, points `hyprqt6engine` at the generated KDE `.colors` file rather than the qtct palette file so KDE's `KColorScheme` path is set, uses the shared system-font and mono-font offset helpers for `hyprqt6engine`, treats `dark_hint` as the KDE/Qt GUI polarity, and resolves a same-family scheme with the requested `appearance` through the shared `desktopctl/src/theme/targets/scheme_pair.rs` resolver (explicit `dark_scheme` pairing wins, then variant-preference order, then alphabetical; `catppuccin-latte` with `dark_hint` now resolves Mocha instead of the alphabetical-accident Frappé) before falling back to the selected scheme. The shared system baseline in `system/configuration.nix` now supplies the runtime half of that contract by enabling NixOS `qt.enable`, exporting `QT_QPA_PLATFORMTHEME=hyprqt6engine`, keeping hyprqt6engine's nonstandard plugin root visible, installing qtct/Kvantum packages system-wide without a global `QT_STYLE_OVERRIDE`, and applying the shared fontconfig tuning that gives `system_font = "SF Pro"` RGB subpixel AA plus an `SF Pro Text` preference. |
...
| `starship` | `desktopctl/src/theme/targets/starship.rs` concats `config/starship/base.toml` and emits the scheme palette as `[palettes.current]`, including `color_blue_bright` (from `colors.blue_bright`), which base.toml's `docker_context` and `conda` segments reference instead of a hardcoded hex value — keeping the base file free of theming values. |
| `vicinae` | `desktopctl/src/theme/targets/vicinae.rs` writes only `~/.config/vicinae/settings.theme.json`, while Home Manager deploys `config/vicinae/settings.json` as the base file that imports that fragment. The generated fragment sets the selected Vicinae dark/light theme names, the shared `icon_theme` under each Vicinae theme variant, and the shared system font; for a light scheme, the dark slot names the same-family dark companion resolved via the shared `scheme_pair` resolver (the old `app_themes.vicinae.light_name` piggyback is gone). Its `persist()` hook writes custom TOML themes under `~/.local/share/vicinae/themes/` using the configured Vicinae theme IDs from `ColorScheme.app_themes.vicinae`, and additionally writes the resolved companion scheme's TOML so the referenced paired file always exists. |
| `vscode` | `desktopctl/src/theme/targets/vscode.rs` merges the repo base settings with theme-owned color, icon-theme, and font keys. The icon theme id is the one contributed by an extension (`catppuccin-mocha`, `gruvbox-material-icon-theme`), which is not the extension id; only Catppuccin's four schemes and both Gruvbox variants declare one, and the rest fall back to the accent-tinted `material-icon-theme`. It and now prefers a `'<mono font> Mono', '<mono font>', monospace` stack for `terminal.integrated.fontFamily` when the selected font is a Nerd Font so prompt glyphs render reliably in the integrated terminal. |
...
| `wallpaper` | `desktopctl/src/theme/targets/wallpaper.rs` keeps the `~/.cache/apply-theme/wallpaper` cache path (deliberate preservation) and `awww` runtime side effects while remaining `sync_safe = false`. The `lutgen` cache key is a 64-bit FNV-1a fingerprint (`theme::fnv1a_fingerprint`, shared with wallpaper-preview naming) under cache version `lutgen-apply-v2`; old v1 entries are orphaned and regenerate on demand. |
| `where_is_my_sddm_theme` | `desktopctl/src/theme/targets/where_is_my_sddm_theme.rs` stages the currently selected wallpaper bytes at `/tmp/desktopctl-where-is-my-sddm-theme/background`, preferring the filtered wallpaper cache when that file already exists, so the root-owned bridge in `system/services.nix` can copy the image into `/var/lib/desktopctl/where-is-my-sddm-theme/background` for SDDM's static `theme.conf.user` path. The staging directory is pre-created at boot by a systemd-tmpfiles rule in `system/services.nix` as `0700 kevin:kevin`, so the target's `create_dir_all` is a no-op and no other local user can plant the directory or a symlink first. |
...
| Chromium | `desktopctl/src/theme/targets/chromium.rs` patches each active profile's `Preferences` file in place so Chromium web content picks up the managed system and mono font selections while page sizes stay on Chromium defaults, and browser chrome uses the managed `dark_hint` through `browser.theme.color_scheme2`. |
| Helium | `desktopctl/src/theme/targets/helium.rs` applies the same Chromium-family preference patching under `~/.config/net.imput.helium/`, including clearing Helium color-picker prefs that otherwise override the desktop hint. |
...
| Hyprland | `config/hypr/hyprland.conf` and `config/hypr/appearance.conf` source generated `colors.conf`, `cursor.conf`, and `appearance-theme.conf`. The `awww-daemon` startup block in `config/hypr/autostart.lua` re-applies the wallpaper target from persisted theme state once the daemon is ready. |
| Quickshell | `config/quickshell/popups/SettingsPopup.qml`, the Fonts/Presets panes, and `config/quickshell/ShellOptions.qml` invoke `desktopctl theme` with structured argv arrays; the former `theme.apply` shell IPC bridge in `config/quickshell/shell.qml` was removed, so the settings host is the only shell-side theme mutation path. The settings host stages individual `theme set` writes optimistically, serializes queued `set` / `preset` requests, and reloads or rolls back on process exit. Shell-side pane structure, shared UI primitives, the live generated `~/.config/quickshell/GeneratedTheme.json` file, and the `Theme.qml` fallback path are documented in `docs/quickshell/ARCHITECTURE.md`. |
...
| SDDM | `system/services.nix` points `where_is_my_sddm_theme` at the persistent root-owned `/var/lib/desktopctl/where-is-my-sddm-theme/background` file with a fixed blur radius, while the `where_is_my_sddm_theme` target stages the current wallpaper in the tmpfiles-pre-created `/tmp` staging directory and the `desktopctl-sddm-theme-sync.path` / `.service` bridge copies it into that persistent path for the greeter. |
