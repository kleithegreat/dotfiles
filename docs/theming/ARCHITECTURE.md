...
| Registry | `desktopctl/src/theme/targets/mod.rs` defines target metadata, including primary generated outputs plus any additional hook-managed filesystem paths, exposes the hook surfaces, and registers all 23 current targets explicitly. |
| Orchestrator | `desktopctl/src/theme/orchestrator.rs` handles dependency selection, target ordering, file assembly, atomic file replacement, concat merges, repo-relative `base_path` resolution, post-write hooks, reload hooks, and sync-safe filtering. It now also exposes the shared color/font target sets reused by the `theme colors` and `theme fonts` CLI paths in `desktopctl/src/theme/mod.rs`, so those apply scopes stay aligned with the orchestrator-owned target inventory. The current dependency map routes `font_size`, `quickshell_font_size_offset`, `gtk_font_size_offset`, and `qt_font_size_offset` back to the matching active font consumers, leaves the persisted legacy `chromium_font_size_offset` key with no fanout, keeps the GtkSourceView color fanout, includes both OpenCode and Zsh in the `color_scheme` fanout, and no longer routes `tmux` through the mono-font dependency path it does not consume. |
...
| Quickshell | `config/quickshell/Theme.qml`, `config/quickshell/popups/SettingsPopup.qml`, the Fonts/Presets panes and preset editor, and `config/quickshell/shell.qml` call or route `desktopctl theme` through argv-safe command construction instead of hardcoded repo scripts. The settings host stages individual `theme set` writes optimistically, serializes queued `set` / `preset` requests, reloads or rolls back on process exit, now exposes the active system-font offset set (`quickshell`, `GTK`, `Qt`) plus the full mono-offset set (including `neovide_mono_font_size_offset`) through the Fonts and Presets panes, keeps `icon_theme` and cursor controls on separate Icons and Mouse panes, and now uses shared preview-card selectors for both color schemes and icon themes, with the icon cards loading representative assets from the installed theme directories instead of drawing generic stand-ins. The recursive Quickshell tree also carries one committed bootstrap snapshot at `config/quickshell/GeneratedTheme.json`, which activation/runtime theme applies overwrite in place. |
