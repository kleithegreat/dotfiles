...
**Cause:** The `chromium` target patches each active profile's `~/.config/chromium/<profile>/Preferences` `webkit.webprefs` entries based on `Local State` `profile.last_active_profiles`, falling back to `Default` when that list is unavailable. Those prefs drive web-content font families and the same integer font-size values Chromium persists in its own settings UI; Chromium's own chrome continues to use the toolkit font settings from GTK, Chromium keeps one prefs file per profile, and a live browser session may rewrite an active profile's file on exit.
**Status:** Current behavior
**Resolution:** Treat the target as owning active profiles' web-font prefs only. Use the GTK font size and GTK offset controls for Chromium's browser chrome, and reapply the Chromium target after closing Chromium if a live session overwrote the managed values. The Quickshell Fonts pane labels the per-target offset as `Chromium pages` to reflect that scope. Open an inactive profile once and rerun the target if you want that profile updated too.
