...
| --- | --- | --- |
| `hosts/laptop/system.nix` | Laptop bootloader policy | The laptop `boot.loader.grub` block enables EFI GRUB with manual chainloader entries for Arch and Windows |
...
| --- | --- | --- |
| `laptop` | `enable = true`, `efiSupport = true`, `device = "nodev"`, `useOSProber = false`, plus `extraEntries` for Arch and Windows | The laptop host module mounts the EFI system partition at `/boot/efi` through `boot.loader.efi.efiSysMountPoint` |
