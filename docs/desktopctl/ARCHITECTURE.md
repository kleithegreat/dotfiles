...
| Theme CLI surface | `desktopctl/src/theme/mod.rs` implements the current `desktopctl theme` command surface, including `all`, `sync`, scoped apply commands, `set`, `preset`, preset-file management, and `status` / `list-*` inspection output. The `colors` and `fonts` CLI paths now reuse the shared target sets exported by `desktopctl/src/theme/orchestrator.rs` instead of carrying duplicate name lists locally. The `list-schemes --json` path emits richer preview objects for Quickshell, including identity, appearance, named colors, bright variants, and the 16-color terminal palette, while `list-wallpapers --json` now emits filename-ordered wallpaper entries with cached preview-image paths for Quickshell. `set_dark_hint()`, `cmd_set()`, and `cmd_preset()` still keep `dark_hint` on the direct theming path, while the theme tests there now cover preserving an explicit `dark_hint` across `color_scheme` changes. |
