...
| Home Manager install and activation | `home/default.nix:38-52` adds `desktopctl` to `home.packages`, and `home/default.nix:333-337` now bootstraps `~/.config/hypr/input-runtime.conf` before running `desktopctl theme sync` during Home Manager activation. |
...
- `desktopctl/src/theme/targets/chromium.rs:220-394` adds focused coverage for
