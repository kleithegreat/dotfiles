...
| Theme schema and validation | `desktopctl/src/theme/schema.rs:33-120`, `desktopctl/src/theme/schema.rs:405-620`, and `desktopctl/src/theme/resolve.rs:11-500` define the `ColorScheme` and `ThemeState` contract, including required scheme `appearance`, centralized app-theme metadata including KTextEditor theme names, compiled default theme-state values, canonical field ordering, per-target system-font and mono-font offsets, color-scheme loading, and `theme_state` persistence with legacy `themes/state.json` import support. `desktopctl/src/theme/schema.rs:444-485` now derives the default `dark_hint` from the default color scheme's declared appearance. |
