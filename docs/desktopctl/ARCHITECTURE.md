...
- `desktopctl/src/solar.rs`, `desktopctl/src/daemon/solar.rs`, and
  `desktopctl/src/daemon/night_light.rs` now cover sunrise/sunset ordering,
  pre-sunrise and post-sunset schedule transitions, the 06:00 `dark_hint`
  cutoff, cached coordinate resolution, scheduler sleep math, the
  controller's auto/manual `hyprsunset` mapping, and the edge-triggered
  scheduled `dark_hint` updates.
