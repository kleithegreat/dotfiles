...
| --- | --- | --- |
| Medium | `dark_hint` still has multiple live policy initiators. | `update_solar_status()` / `reconcile_locked()` in `desktopctl/src/daemon/night_light.rs` now issue the scheduled 23:00 enable and 06:00 disable, but `set_dark_hint()`, `cmd_set()`, and `cmd_preset()` in `desktopctl/src/theme/mod.rs` still let `theme set dark_hint ...` and preset-supplied `dark_hint` values persist directly without daemon arbitration. Docs in this domain and in `docs/sun-schedule/` must treat that split as current behavior, not as resolved delegation. |
