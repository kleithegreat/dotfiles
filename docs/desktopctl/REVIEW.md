...
| --- | --- | --- |
| Medium | `dark_hint` still has multiple live policy initiators. | `update_solar_status()` / `reconcile_locked()` in `desktopctl/src/daemon/night_light.rs` issue the nightly `dark_hint` enable, but `set_dark_hint()`, `cmd_set()`, and `cmd_preset()` in `desktopctl/src/theme/mod.rs` still let `theme set dark_hint ...` and preset-supplied `dark_hint` values persist directly without daemon arbitration. Docs in this domain and in `docs/sun-schedule/` must treat that split as current behavior, not as resolved delegation. |
