...
**Resolution:** Treat `desktopctl night-light ...` as the `hyprsunset` control surface and `desktopctl theme set dark_hint ...` as the persisted browser/GTK hint surface. The daemon startup reconciliation plus scheduled 23:00 enable / 06:00 disable are the only automatic bridge between them; there is still no single unified override model.
