...
**Cause:** `desktopctl/src/daemon/night_light.rs:129-163` only applies `dark_hint` while mode is `auto`, while `desktopctl/src/theme/mod.rs:73-95` and `desktopctl/src/theme/mod.rs:318-388` still allow direct `dark_hint` writes from theme surfaces.
