...
| Laptop GPU routing locality | The laptop Hyprland env keeps Intel-first user-session routing localized to `config/hypr/env.lua`, so the hybrid-specific `LIBVA_DRIVER_NAME` and `AQ_DRM_DEVICES` policy are easy to audit. |
...
- The laptop Hyprland path intentionally stays on `cardN` ordering:
  `config/hypr/env.lua` uses `/dev/dri/card2:/dev/dri/card1` because
