...
fragments. The main improvement targets are to stop encoding laptop-specific EGL
policy in the shared baseline, replace brittle laptop DRM device numbering, and
make the desktop resume workaround lifecycle easier to audit.

## Strengths

| Area | Current state |
| --- | --- |
| Host split | The repo cleanly separates the hybrid laptop path from the dedicated desktop path at the host-module layer (`hosts/laptop/system.nix:46-62`, `hosts/desktop/system.nix:56-75`). |
...
| Medium | The shared baseline still carries laptop-specific Mesa-only EGL policy. | `system/configuration.nix:214-215` sets `__EGL_VENDOR_LIBRARY_FILENAMES` globally to Mesa-only, and `hosts/desktop/system.nix:69-75` has to `lib.mkForce` it back to a dual-vendor value. That makes the desktop depend on overriding a shared default that is not actually host-agnostic. |
...
- The Mesa-only shared EGL setting plus the desktop override are still present
  in `system/configuration.nix:214-215` and `hosts/desktop/system.nix:69-75`.
