...
| Hybrid laptop GPU policy | `hosts/laptop/system.nix` plus `config/hypr/env.lua` | Own the PRIME, Xorg driver list, laptop-local kernel GPU-driver pruning, Mesa EGL vendor selection, and Hyprland user-session GPU env for the laptop |
...
| Hyprland env file | `config/hypr/env.lua` | `hosts/desktop/env.conf` |
