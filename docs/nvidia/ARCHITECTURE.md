...
Hyprland session environment as of 2026-04-29.
...
| `hosts/laptop/system.nix` | Hybrid laptop kernel and X stack | The laptop `hardware.nvidia` block enables the open NVIDIA driver with `modesetting`, enables NVIDIA power management plus `powerManagement.finegrained`, configures PRIME offload, uses `services.xserver.videoDrivers = [ "modesetting" "nvidia" ]`, sets a laptop-only Mesa EGL vendor list, and disables `DRM_NOUVEAU` through the laptop-only `boot.kernelPatches` config |
