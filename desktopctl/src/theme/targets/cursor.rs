...
use std::{collections::BTreeMap, fs, io, path::Path, process::Command};
...
.managed_paths(&[
    "~/.config/hypr/cursor-theme.lua",
    "~/.local/share/icons/default/index.theme",
    "~/.icons/default/index.theme",
])
.comment("#");

const HYPR_CURSOR_LUA: &str = "~/.config/hypr/cursor-theme.lua";
...
/// Quote a value as a Lua string literal for the generated cursor data table.
...
// BreezeX-RosePine-Linux is an XCursor theme; its hyprcursor build is the
// separate rose-pine-hyprcursor package installed via home/packages.nix.
...
fn replace_with_symlink(
    ...
    if crate::theme::replace_with_symlink(link_path, target).is_ok() {
        return Ok(());
    }
...
pub fn persist(_colors: &ColorScheme, state: &ThemeState) -> crate::Result<()> {
    ...
    let conf_path = expand_user_path(HYPR_CURSOR_LUA)?;

    if let Some(parent) = conf_path.parent() {
        fs::create_dir_all(parent)?;
    }
    let mut lines = vec![
        "-- Generated by desktopctl theme -- do not edit".to_owned(),
        "return {".to_owned(),
        format!("    XCURSOR_THEME = {},", lua_str(&state.cursor_theme)),
        format!("    XCURSOR_SIZE = {},", lua_str(&state.cursor_size.to_string())),
    ];
    if let Some(hyprcursor_theme) = hyprcursor_env_theme(&state.cursor_theme) {
        lines.push(format!(
            "    HYPRCURSOR_THEME = {},",
            lua_str(hyprcursor_theme)
        ));
    }
    lines.push("}".to_owned());
...
pub fn on_apply(_colors: &ColorScheme, state: &ThemeState) -> crate::Result<()> {
    // Collect failures instead of early-returning so a missing dconf or
    // hyprctl does not prevent the remaining runtime updates from applying.
    let mut failures = Vec::new();
    let mut attempt = |result: crate::Result<()>| {
    ...
    attempt(dconf_set(
        "cursor-theme",
        &format!("'{}'", state.cursor_theme),
    ));
    attempt(dconf_set("cursor-size", &state.cursor_size.to_string()));
    ...
    attempt(hyprctl_env("XCURSOR_THEME", &state.cursor_theme));
    attempt(hyprctl_env("XCURSOR_SIZE", &state.cursor_size.to_string()));

    if let Some(hyprcursor_theme) = hyprcursor_theme {
        attempt(hyprctl_env("HYPRCURSOR_THEME", hyprcursor_theme));
        session_env.insert("HYPRCURSOR_THEME".to_owned(), hyprcursor_theme.to_owned());
    } else {
        attempt(hyprctl_env("HYPRCURSOR_THEME", ""));
    }
