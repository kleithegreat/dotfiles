...
FocusScope {
    ...
    Keys.onPressed: (event) => {
        if (event.key === Qt.Key_Escape && root.expanded) {
            root.expanded = false;
            event.accepted = true;
        } else if ((event.key === Qt.Key_Return || event.key === Qt.Key_Enter || event.key === Qt.Key_Space) && root.activeFocus) {
    ...
    Column {
        ...
        Rectangle {
            ...
            color: triggerArea.containsMouse || root.expanded ? Root.Theme.bg2 : Root.Theme.bg1
            border.width: 1
            border.color: root.expanded ? Root.Theme.accent : (triggerArea.containsMouse ? Root.Theme.fg4 : Root.Theme.bg3)
